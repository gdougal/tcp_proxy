...
uint32_t					Config::Section::getAddrVal(const std::string &Key)		const { return inet_addr(throw_or_not(Key).c_str()); }
uint16_t					Config::Section::getPortVal(const std::string &Key)		const { return htons(static_cast<uint16_t>(std::stoul(throw_or_not(Key)))); }
